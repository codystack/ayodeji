    <header id="header-sticky" class="header-section absolute-bg border-0 style1 position-relative w-100">
        <div class="container pe-0 ps-0">
            <div class="mega-menu-wrapper">
                <div class="header-main">
                    <div class="header-left">
                        <div class="logo">
                            <a href="./" class="header-logo">
                                <img src="./assets/img/logo-dark.png" width="200" alt="logo-img">
                            </a>
                        </div>
                    </div>
                    <div class="header-right d-flex justify-content-end align-items-center">
                        <div class="mean__menu-wrapper">
                            <div class="main-menu">
                                <nav id="mobile-menu">
                                    <ul>
                                        <li><a href="./">Home</a></li>
                                        <li><a href="about">About</a></li>
                                        <li>
                                            <a href="#">Services</a>
                                            <ul class="submenu">
                                                <li><a href="executive-coaching">Executive Coaching</a></li>
                                                <li><a href="business-coaching">Business Coaching</a></li>
                                            </ul>
                                        </li>
                                        <li><a href="">Shop</a></li>
                                        <li><a href="contact">Contact</a></li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
                        <div class="d-flex align-items-center gap-3">
                            <div class="header__hamburger d-xl-none d-block my-auto">
                                <div class="sidebar__toggle">
                                    <img src="./assets/img/bars.png" alt="icon">
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="header-hamburger-inner d-xl-flex align-items-center d-none">
                        <a href="https://calendly.com/ayodeleolajiga/free-executive-coaching-strategy-call" target="_blank" class="theme-btn text-nowrap style3 pe-20">
                            <i class="fa-solid fa-arrow-right w-36 h-36 bg-white rounded-circle d-center fz-14 theme-clr4"></i>
                            Book a consultation
                        </a>
                        <div class="header__hamburger my-auto d-xl-none d-block">
                            <div class="sidebar__toggle">
                                <img src="./assets/img/bars.png" alt="icon">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </header>